...
export interface RollCard {
  ...
  check?: { total: number; dv: number; label: string };
  detail?: string;
  effect?: string;
}
export function rollCard(data: RollCard): string {
  ...
  return `<div class="rollcard crewtools-roll"><div class="rollcard-top"><div class="cpr-block crewtools-roll-card">
    ...
    ${data.check ? `<div class="crewtools-roll-check"><span>${escape(data.check.label)}</span><strong>${escape(data.check.total)} <small>vs DV ${escape(data.check.dv)}</small></strong></div>` : ""}
