...
HQ contributions settle automatically through the primary connected GM. With no GM online, a player can settle their own contributions only when they are the sole connected user and the HQ page permits editing. When multiple players are online without a GM, contributions remain Processing until a GM connects or only one player remains. No GM approval is required. Automatic reconciliation on reconnect recovers unfinished contributions. Completed payments reduce outstanding rent on the HQ Journal page. If pending contributions exceed the remaining bill, only the outstanding amount is applied and the rest is refunded to the contributor's native money ledger. Reprocessing a confirmed receipt cannot credit the HQ twice. Successful payments and rollbacks explicitly clear the temporary payment marker. For existing interrupted records, the owner can select **Resolve Payment Issue** and confirm **Allow Payments Again**. This clears only the block; money, bills and receipts remain unchanged. Check the native ledger and paid bills before paying again.
